...
--*/
{
    ...
    switch(NotifyClass) {
        ...
        case RegNtPreSetValueKey:
            ...
            if (!NT_SUCCESS (Status)) {
                DbgPrint("REGFLTR ### ObObjectByPointer failed. Status 0x%x\n", Status);
                break;
            } 
            ...
            if(!NT_SUCCESS(Status)) {
                DbgPrint("REGFLTR ### ZwSetValue in CallbackModify failed. Status 0x%x\n", Status);
                ZwClose(RootKey);
                break;
            }
            ...
            DbgPrint("REGFLTR ### \tCallback: Set value %wZ bypassed.\n", PreSetValueInfo->ValueName);
        ...
        case RegNtPreDeleteValueKey:
            ...
            if(!NT_SUCCESS(Status)) {
                DbgPrint("REGFLTR ### ZwDeleteValue failed. Status 0x%x\n",
                           Status);
                ZwClose(RootKey);
                break;
            }
            ...
            DbgPrint("REGFLTR ### \tCallback: Delete value %S bypassed.\n", ValueName->Buffer);
            Status = STATUS_CALLBACK_BYPASS;
            ZwClose(RootKey);
            break;
...
--*/
{
    ...
    if (TempBuffer != NULL) {
        try {
            ...
            DbgPrint("REGFLTR ### Capturing buffer failed with exception\n");
            ExFreePoolWithTag(TempBuffer, PoolTag);
            TempBuffer = NULL;
            Status = GetExceptionCode();
        }
    } else {
        DbgPrint("REGFLTR ### Capturing buffer failed wtih insufficient resources\n");
...
--*/
{
    ...
    if (DestString->Buffer != NULL) {
        ...
        try {
            ...
            DbgPrint("REGFLTR ### Capturing Unicode String failed with exception\n");
        ...
        DbgPrint("REGFLTR ### Capturing Unicode String failed wtih insufficient resources\n");
        Status = STATUS_INSUFFICIENT_RESOURCES;
    }
