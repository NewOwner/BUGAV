...
VOID FilterFileCtrl::Passflt_LoadDriver() {
    BOOL Result;
    Result = FilterCommonCtrl::UtilLoadDriver((LPTSTR)PASSFLT_DRIVER_NAME,
        (LPTSTR)PASSFLT_DRIVER_NAME_WITH_EXT,
        (LPTSTR)PASSFLT_WIN32_DEVICE_NAME,
        &hDriver);
...
VOID FilterFileCtrl::Passflt_UnloadDriver() {
    BOOL Result;
    Result = FilterCommonCtrl::UtilUnloadDriver(hDriver, NULL, (LPTSTR)PASSFLT_DRIVER_NAME);
}

VOID FilterFileCtrl::Passflt_OpenDevice() {
    BOOL ReturnValue = FilterCommonCtrl::UtilOpenDevice((LPTSTR)PASSFLT_WIN32_DEVICE_NAME, &hDriver);
...
VOID FilterFileCtrl::Passflt_UpdateConfig() {
    Passflt_SendMessage((PCHAR)PASSFLT_UPD_CFG_MSG);
}

VOID FilterFileCtrl::Passflt_ConnectCommunicationPort() {
...
VOID FilterFileCtrl::Passflt_SendMessage(PCHAR msg) {
